...
\[
  \underbrace{\bm{Y} = \bigl(\bm{L}\odot\bm{C}\bm{B}^\top\bigr)\bm{V}}_{\text{Mamba-3 SSD}}
...
\]
i.e.\ the \emph{query} projection becomes $\bm{C}$, the \emph{key}
projection becomes $\bm{B}$, and the \emph{value} projection is the
usual learned $\bm{W}_V$ output (with the SiLU pre-gate from
\eqref{eq:projV}; note this is the projected value, not the raw input
