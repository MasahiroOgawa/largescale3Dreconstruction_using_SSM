...
\begin{itemize}
  \item \textbf{Scale-normalised relative position L1} on visible frames:
        \begin{equation}\label{eq:track-pos-loss}
          \mathcal{L}_{\text{pos}} \;=\;
          \frac{1}{Z_{\text{pos}}}
          \sum_{n,t} \sigma_n^{\star,(t)} \,\|\bm{r}_n^{(t)}\|_{1},
        \end{equation}
        with normaliser $Z_{\text{pos}} = \sum_{n,t}
        \sigma_n^{\star,(t)}$. The combined effect of
        \eqref{eq:track-resid} and the median scale
        \eqref{eq:track-scale} makes the loss \emph{dimensionless} and
        clip-scale-invariant: every clip contributes a comparable
        gradient regardless of indoor/outdoor scene metres.
  \item \textbf{Visibility BCE}:
        $\mathcal{L}_{\text{vis}} = \frac{1}{Z_{\text{q}}} \sum_{n,t}
        \mathrm{BCE}\bigl(\sigma_n^{(t)},\sigma_n^{\star,(t)}\bigr)$
        with $Z_{\text{q}}$ the count of valid (clip,track,frame) cells.
  \item \textbf{Spawn BCE} on the first-visible frame:
        $\mathcal{L}_{\text{spawn}} = \frac{1}{Z_{\text{q}}}
        \sum_{n,t} \mathrm{BCE}\bigl(\rho_n^{(t)},
        \mathbb{I}[t = t_n^{\text{birth}}]\bigr)$ where
        $t_n^{\text{birth}} = \min\{t : \sigma_n^{\star,(t)} = 1\}$.
  \item \textbf{Temporal smoothness} on the scaled relative trajectory:
        $\mathcal{L}_{\text{smooth}} = \frac{1}{Z_{\text{sm}}}
        \sum_{n,t}
        \sigma_n^{\star,(t)}\sigma_n^{\star,(t-1)}
        \,\|\tfrac{1}{s}(\Delta\bm{p}_n^{(t)} - \Delta\bm{p}_n^{(t-1)})\|_{2}$.
\end{itemize}
The total loss is
...
with default weights $(1,\,0.5,\,0.5,\,0.1)$.
