...
three small MLPs:
\begin{align}
  \bm{p}_n^{(t)} &\;=\; \mathrm{MLP}_{\text{xyz}}(\bm{q}_n^{(t)}) \;\in\; \mathbb{R}^{3},
  & &\text{(3D position)}\\
  ...
  & &\text{(spawn flag)}
\end{align}
where $\sigma$ is the sigmoid. Stacking over all $N$ tracks gives the
predicted trajectory tensor
\begin{equation}
  \bigl(\bm{p}^{(t)},\,\bm{\sigma}^{(t)},\,\bm{\rho}^{(t)}\bigr)
...
The fixed-size bank $\bm{Q}\in\mathbb{R}^{N\times D}$ accommodates a
worst-case track count. As in Mask2Former (\S\ref{sec:seg-C}), $N$ is
chosen well above the typical instance count (e.g.\ $N=256$ for
ordinary dynamic scenes).
...
\textbf{Spawn.} A track is born at frame $t$ when $\rho_n^{(t)} >
\tau_{\text{spawn}}$ on a query whose visibility was previously zero.
At training time, ground-truth spawn events are assigned to
currently-dormant queries via Hungarian matching, exactly as in the
DETR / Mask2Former pipeline; at inference, the $\rho$ head decides
which dormant queries to wake up.

\textbf{End of life.} Tracks that go invisible for more than $L_{\max}$
frames (e.g.\ 30) may be re-claimed by the spawn logic: their query
embedding is reset to a learned ``free-slot'' template before the
next frame's cross-attention, so a long-departed track does not
permanently occupy a slot.

\subsection{Loss}
\label{sec:track-loss}

Let $\bm{p}_n^{\star,(t)}$, $\sigma_n^{\star,(t)}$ be the
ground-truth 3D position and visibility for track $n$ at frame $t$
(after a Hungarian matching has assigned predicted queries to GT
tracks at frame 1). Train with the sum of
\begin{itemize}
  \item Per-frame, per-track 3D regression on visible frames:
        $\sum_{n,t} \sigma_n^{\star,(t)}\,\bigl\|\bm{p}_n^{(t)} -
        \bm{p}_n^{\star,(t)}\bigr\|_{1}$.
  \item Visibility BCE: $\sum_{n,t}
        \mathrm{BCE}\bigl(\sigma_n^{(t)},\sigma_n^{\star,(t)}\bigr)$.
  \item Spawn BCE on the birth frames only:
        $\sum_{n,t} \mathbb{I}[t = t_n^{\text{birth}}]\,
        \mathrm{BCE}(\rho_n^{(t)}, 1)
        + \mathbb{I}[t \neq t_n^{\text{birth}}]\,
        \mathrm{BCE}(\rho_n^{(t)}, 0)$.
  \item Optionally a smoothness prior
        $\sum_{n,t} \sigma_n^{\star,(t)}\sigma_n^{\star,(t-1)}\,
        \|\bm{p}_n^{(t)} - \bm{p}_n^{(t-1)}\|_{2}$
        to discourage trajectory jitter on visibly continuous segments.
\end{itemize}
