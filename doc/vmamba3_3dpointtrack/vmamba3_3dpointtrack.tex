...
\begin{tikzpicture}[
  ...
  \draw[arr] (head) -- (mul);
  \draw[arr] (mul) -- (z);
  % z_raw bypass arrow: down from zraw.south, right below all boxes, up to mul.south
  \coordinate (bypasswa) at ($(zraw.south)+(0,-3mm)$);
  \draw[arr,rounded corners=2pt] (zraw.south) -- (bypasswa) -- (bypasswa -| mul.south) -- (mul.south);
...
\begin{tikzpicture}[
  ...
  \node[block, below right=5mm and 35mm of ssm] (duv) {$\Delta\mathbf{u}$~head\\{\scriptsize 0-init, $\tanh\!\cdot\!2$}};
  % ── outputs ───────────────────────────────────────────────────────
...
\begin{table}[h]
\centering
\caption{Inference throughput on RTX\,4080 (12\,GB). Our lightweight refiner adds negligible
         cost over the baseline; TrackCraft3R's diffusion backbone is ${\approx}200\times$ slower.}
...
\end{tabular}
\\[2pt]
{\footnotesize $^{\S}$ TrackCraft3R is a pre-trained 1.3\,B-parameter video diffusion model (Wan2.1
DiT $+$ 3 VAEs $+$ LoRA); it is not trained by us, hence ``---'' for trainable params.
Throughput measured on the same RTX\,4080 via windowed inference (see text).}
\end{table}
...
and the 0.40\,M-parameter SSM adds negligible overhead over the frozen SEA-RAFT $+$ DA3 baseline.
Combined with the accuracy advantage on the absolute metric
(Table~\ref{tab:abs}: mean metric-AJ $0.234$ vs $0.025$ for TrackCraft3R on pstudio),
