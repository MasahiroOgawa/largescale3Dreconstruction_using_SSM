...
\begin{itemize}
\item \textbf{DINOv3 appearance features} (64-D): sampled from a frozen ViT-S/16 DINOv2
  backbone at the current track position.  These provide a local visual ``fingerprint'' of the
  tracked object, allowing the SSM to detect when the 2D position has drifted off the target.
\item \textbf{Local depth patch} ($5{\times}5 = 25$ values): DA3 depths in a neighbourhood of
  the track point, normalised to the centre depth.  This directly exposes depth discontinuities
  at object boundaries, the very features that caused v32 to overshoot.
\end{itemize}

A second output head (also zero-initialised) predicts a 2D correction $\Delta\mathbf{u}$, bounded
by $\tanh(\cdot)\!\cdot\!2$ to $\pm 2$\,px.  Crucially, after $\Delta\mathbf{u}$ shifts the pixel,
...
\begin{tikzpicture}[scale=1.0]
  ...
  \node[draw=orange!70!black,fill=orange!8,rounded corners,font=\scriptsize,inner sep=2pt,align=center]
    at (0.9,-1.05) {DINOv3\\feat (64-D)};
  \draw[-{Stealth[length=1.8mm]},orange!70!black] (0.9,-0.75) -- (0.9,-0.06);
  % depth patch annotation
  \node[draw=green!60!black,fill=green!8,rounded corners,font=\scriptsize,inner sep=2pt,align=center]
    at (2.4,-1.05) {depth patch\\$5{\times}5$};
  \draw[-{Stealth[length=1.8mm]},green!60!black] (2.4,-0.75) -- (2.2,0.23);
  % bottom note
  \node[font=\scriptsize,align=left] at (2.0,-1.65)
    {New SSM inputs (orange/green) $\Rightarrow$ informed $\Delta\mathbf{u}$ avoids depth-discontinuity artefacts.
     Depth is re-sampled at $\mathbf{u}_0{+}\Delta\mathbf{u}$ before applying $\Delta\!\log z$.};
\end{tikzpicture}
\caption{v35 joint correction. Two new inputs (DINOv3 appearance features and a local $5{\times}5$
depth patch) give the SSM the context needed to predict $\Delta\mathbf{u}$ safely.
Unlike v32, which nudged the pixel blindly and crossed depth discontinuities, v35's 2D correction
is conditioned on local appearance and local depth structure.
Depth is re-sampled at the shifted pixel before $e^{\Delta\!\log z}$ rescales it.}
...
\begin{tikzpicture}[
  ...
  % ── inputs (top row) ──────────────────────────────────────────────
  \node[io] (in1)   {$[\mathbf{r}(\mathbf{u}_0),\,z/z_r,\,v]$\\{\scriptsize 4-D}};
  % ── u_0: 2-D flow from SEA-RAFT (see Fig.~\ref{fig:v36-flow}) ────
  \node[io, above=5mm of in1] (uv0) {$\mathbf{u}_0$ (SEA-RAFT)\\{\scriptsize 2-D flow}};
  \node[new, right=6mm of in1]   (patch) {depth patch\\{\scriptsize $5{\times}5$}};
  \node[new, right=6mm of patch] (dino)  {DINOv3 feat\\{\scriptsize 64-D}};
  % ── embed ─────────────────────────────────────────────────────────
  \node[block, below=14mm of patch] (emb) {MLP embed\\{\scriptsize 128-D}};
  % ── SSM ───────────────────────────────────────────────────────────
  \node[learn, right=8mm of emb] (ssm) {Mamba-3 SSD\\{\scriptsize $\times 2$, causal}};
  % ── output heads ──────────────────────────────────────────────────
  \node[block, above right=10mm and 8mm of ssm] (duv) {$\Delta\mathbf{u}$~head\\{\scriptsize 0-init, $\tanh\!\cdot\!2$}};
  \node[block, below right=10mm and 8mm of ssm] (dz)  {$\Delta\!\log z$~head\\{\scriptsize 0-init}};
  % ── outputs ───────────────────────────────────────────────────────
  ...
  % ── depth map bypass ──────────────────────────────────────────────
  \node[io, below=8mm of dz] (dm) {depth map (DA3)};
  % ── arrows ────────────────────────────────────────────────────────
  \draw[arr] (uv0) -- (in1);
  ...
  \draw[arr] (emb) -- (ssm);
  \coordinate (ssmfork) at ($(ssm.east)+(4mm,0)$);
  \draw (ssm.east) -- (ssmfork);
  \fill (ssmfork) circle[radius=1pt];
  \draw[arr] (ssmfork) -| (duv.west);
  \draw[arr] (ssmfork) -| (dz.west);
  \draw[arr] (duv) -- (uvout);
  \draw[arr] (dz)  -- (zout);
  \draw[arr] (dm.east) to[bend left=25] (zout.south);
  \node[font=\scriptsize,gray,align=center,above=1.5mm of uvout]
    {\footnotesize re-sample DA3 depth at $\mathbf{u}_t$};
\end{tikzpicture}
\caption{v35 architecture (extends Fig.~\ref{fig:arch}).
...
All three inputs are concatenated and fed through the Mamba-3 SSD layers (the same
causal SSD stack as v33, now carrying appearance and local-geometry context).
A second head produces $\Delta\mathbf{u}$ (bounded to $\pm 2$\,px by $\tanh$).
...
v36 uses the same v35 architecture but runs SEA-RAFT twice --- once forward and once on the
time-reversed video --- and averages the two 2D track estimates before passing them to the SSM
(Fig.~\ref{fig:v36-flow}).
...
\begin{tikzpicture}[
  ...
  % two RAFT branches
  \node[fwd, above right=8mm and 8mm of vid] (fwd) {SEA-RAFT\\(forward)};
  \node[fwd, below right=8mm and 8mm of vid] (bwd) {SEA-RAFT\\(reversed video)};
  % merge
  \node[mrg, right=14mm of vid, yshift=0mm]
    (merge) at ($(fwd.east)!0.5!(bwd.east)+(8mm,0)$) {average\\$+$ flip time};
  % v35 SSM
  \node[learn, right=8mm of merge] (v35) {v35 SSM\\(Fig.~\ref{fig:v35-arch})};
  % output
  \node[io, right=6mm of v35] (out) {3D tracks\\$(F,N,3)$};
  % arrows
  \coordinate (vidfork) at ($(vid.east)+(4mm,0)$);
  \draw (vid.east) -- (vidfork);
  \fill (vidfork) circle[radius=1pt];
  \draw[arr] (vidfork) -| (fwd.west);
  \draw[arr] (vidfork) -| (bwd.west);
  \draw[arr] (fwd.east) -| (merge.north);
  \draw[arr] (bwd.east) -| (merge.south);
  \draw[arr] (merge) -- (v35);
  \draw[arr] (v35) -- (out);
\end{tikzpicture}
\caption{v36 pipeline. The v35 SSM is unchanged; SEA-RAFT runs on both the original and the
time-reversed video, the two 2D track estimates are averaged (and the reversed-video result is
flipped back to forward time), then passed to v35 as the starting 2D positions.
Bidirectional flow improves coverage of points that are temporarily occluded in the forward
direction but visible in the backward pass.}
\label{fig:v36-flow}
\end{figure}

The gain from v36 over v35 is small on all three subsets (Table~\ref{tab:median},
Table~\ref{tab:abs}): bidirectional flow helps drivetrack (moving camera; occlusions are common)
but the pstudio improvement is marginal (fixed cameras; occlusion patterns are nearly symmetric).
...
\bibitem{trackcraft3r} Nam et al., \textit{TrackCraft3R: Repurposing Video Diffusion Transformers for Dense 3D Tracking}, arXiv:2605.12587, 2025.
