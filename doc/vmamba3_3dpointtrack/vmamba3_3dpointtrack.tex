...
\begin{tikzpicture}[
  ...
  % ── inputs (top row) ──────────────────────────────────────────────
  \node[io] (in1)   {$[\mathbf{r},\,z/z_r,\,v]$\\{\scriptsize 4-D (same as v33)}};
  ...
  % ── SSM ───────────────────────────────────────────────────────────
  \node[learn, right=8mm of emb] (ssm) {Mamba-3\\{\scriptsize $\times 2$, causal}};
  % ── output heads ──────────────────────────────────────────────────
  ...
  \draw[arr] (dino.south)  |- (emb.east);
  \draw[arr] (emb) -- (ssm);
  \draw[arr] (ssm.east) |- (duv.west);
  \draw[arr] (ssm.east) |- (dz.west);
  \draw[arr] (duv) -- (uvout);
  \draw[arr] (dz)  -- (zout);
...
The DINOv3 feature and depth patch are concatenated with the 4-D track feature
and fed to the same Mamba-3 stack.
...
\begin{tikzpicture}[
  ...
  % two RAFT branches
  \node[fwd, above right=10mm and 12mm of vid] (fwd) {SEA-RAFT\\(forward)};
  \node[fwd, below right=10mm and 12mm of vid] (bwd) {SEA-RAFT\\(reversed video)};
  % merge
  \node[mrg, right=14mm of vid, yshift=0mm]
    (merge) at ($(fwd.east)!0.5!(bwd.east)+(14mm,0)$) {average\\$+$ flip time};
  % v35 SSM
  \node[learn, right=12mm of merge] (v35) {v35 SSM\\(Fig.~\ref{fig:v35-arch})};
  % output
  \node[io, right=10mm of v35] (out) {3D tracks\\$(F,N,3)$};
  % arrows
  \draw[arr] (vid.east) |- (fwd.west);
  \draw[arr] (vid.east) |- (bwd.west);
