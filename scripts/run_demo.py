...
def run_cross_attention_visual(
    ...
    # Build 2D positions on the patch grid
    ...
    kv_tokens = feats[1:2]  # (1, N, C)
    pos_b = pos.unsqueeze(0)  # (1, N, 2)

    with torch.no_grad():
        _y, attn = cross(q_tokens, kv_tokens, q_pos=pos_b, kv_pos=pos_b, rope=rope, return_attn=True)
    # Pick a centre-ish query token
    query_index = (h // 2) * w + (w // 2)
    save_cross_attention_heatmap(
        attn_map=attn,  # (1, H_heads, T_q, T_kv)
        kv_image=images[1],
        kv_grid_hw=(h, w),
...
def main() -> None:
    ...
    result = overfit_run(net, batch, iters=args.overfit_iters, lr=3e-3, device="cpu")
