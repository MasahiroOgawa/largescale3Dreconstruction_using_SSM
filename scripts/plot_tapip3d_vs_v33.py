...
Generates:
  outputs/plots/comparison_median_aj.png  — median-scaled 3D-AJ per subset
  outputs/plots/comparison_metric_aj.png  — absolute metric-AJ per subset
  outputs/plots/comparison_err_mean.png   — absolute mean 3D error per subset

Usage:
    uv run python scripts/plot_tapip3d_vs_v33.py \\
        --tapip3d-abs-dir outputs/tapip3d_absolute_eval_YYYYMMDD-HHMM
"""
...
# Per-subset results from saved eval files (outputs/metric3d_*/metric_results/*.json)
...
def _load_tapip3d_abs(abs_dir: Path) -> tuple[dict, dict]:
    ...
    for sub in SUBSETS:
        ...
        metric_aj[sub] = float(np.mean([c["metric_average_jaccard"] for c in clips])) * 100
...
def bar_group(ax, groups, series, ylabel, title, pct=True, log_scale=False):
    ...
    for i, (label, vals) in enumerate(series.items()):
        ...
        bars = ax.bar(x + (i - n_series / 2 + 0.5) * w, ys, w * 0.9,
...
def main():
    ...
    ap.add_argument("--out-dir", type=Path, default=REPO / "outputs" / "plots")
    args = ap.parse_args()
    ...
    for tag, med, mj, em in [
        ...
        print(f"  {tag} median-AJ:   {med['drivetrack']:>11.2f}% {med['pstudio']:>11.2f}% {med['adt']:>11.2f}% {np.mean(list(med.values())):>11.2f}%")
        print(f"  {tag} metric-AJ:   {mj['drivetrack']:>11.2f}% {mj['pstudio']:>11.2f}% {mj['adt']:>11.2f}% {np.mean(list(mj.values())):>11.2f}%")
        print(f"  {tag} err_mean:    {em['drivetrack']:>11.3f}m {em['pstudio']:>11.3f}m {em['adt']:>11.3f}m {np.mean(list(em.values())):>11.3f}m")

    args.out_dir.mkdir(parents=True, exist_ok=True)

    # Plot 1: Median-scaled 3D-AJ
    fig, ax = plt.subplots(figsize=(8, 5))
    bar_group(ax, SUBSETS,
