...
from mamba3_tracker.eval.tapvid3d_eval import (
    aggregate, compute_clip_metrics, compute_clip_metrics_absolute,
)
...
def main() -> int:
    ...
    for sub in args.subsets:
        ...
        for path in clips:
            try:
                ...
                rec = {"clip_id": clip.clip_id, **med,
                       "metric_average_jaccard": ab["metric_average_jaccard"],
                       "metric_average_pts_within_thresh": ab["metric_average_pts_within_thresh"],
                       "metric_err_mean_m": mean_m, "metric_err_median_m": median_m}
                per_clip.append(rec)
                print(f"[metric3d] {sub}/{clip.clip_id}: AJ={med['average_jaccard']:.4f} "
                      f"mAJ={ab['metric_average_jaccard']:.4f} "
                      f"mAPD={ab['metric_average_pts_within_thresh']:.4f} "
                      f"err={mean_m:.2f}m(med {median_m:.2f})", flush=True)
            except Exception as e:
                n_fail += 1
                print(f"[metric3d] {sub}/{path.stem}: FAIL {type(e).__name__}: {e}", flush=True)
    ...
    metrics_json = {"method": args.method, "split": args.split,
                    "ckpt": str(args.ckpt) if args.ckpt else None,
                    "per_subset": summary, "overall": overall,
                    "frames": n_frames_total, "elapsed_s": elapsed, "fps": fps,
                    "failures": n_fail}
    ...
    for sub, mm in summary.items():
        rows.append(f"| {sub} | {_g(mm,'average_jaccard'):.4f} | {_g(mm,'average_pts_within_thresh'):.4f} | "
    ...
    rows.append(f"| **mean** | **{_g(overall,'average_jaccard'):.4f}** | "
