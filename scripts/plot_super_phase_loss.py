...
PAT = re.compile(
    ...
    r"L_grad=([-\d.]+)\s+L_P=([-\d.]+)\s+L_C=([-\d.]+)(?:\s+L_feat=([-\d.]+))?"
)
...
def parse(path: Path):
    ...
    for ln in path.read_text().splitlines():
        ...
        if match:
            ...
            f.append(float(match.group(8)) if match.group(8) else float("nan"))
    return s, l, d, m, g, p, c, f
...
def main() -> None:
    ...
    ax_total = axes[0, 0]; ax_d = axes[0, 1]; ax_m = axes[0, 2]; ax_f = axes[0, 3]
    ...
    for log_path, sub_label, color in zip(args.logs, sub_labels, colors):
        s, l, d, m, g, p, c, f = parse(log_path)
        if not s:
        ...
        ax_f.plot(s_g, f, color=color)
        cumulative += max(s) + 1
        boundaries.append(cumulative)

    for ax, title in [(ax_total, "Total"), (ax_d, "L_D (depth)"),
                      (ax_m, "L_M (ray)"), (ax_f, "L_feat (intermediate)"),
                      (ax_g, "L_grad"),
