...
Runs on drivetrack and pstudio (no built-in depth_preds).
ADT has sensor depth_preds, so we skip it here (see make_depthn_annotations.py).

Outputs:
    outputs/tapip3d_annotations/<subset>_megasam_minival/megasam/<seq_id>.h5
...
CU13 = str(TAPIP3D_ROOT / ".venv" / "lib" / "python3.11" / "site-packages" / "nvidia" / "cu13")
...
def run_megasam(frames_dir: Path, output_npz: Path, fov_x: float, resolution: int = 196608) -> bool:
    ...
    env["LD_LIBRARY_PATH"] = ":".join([
        str(TAPIP3D_ROOT / ".venv" / "lib" / "python3.11" / "site-packages" / "torch" / "lib"),
        CU13 + "/lib",
    ...
    cmd = [
        VENV_PYTHON, str(inference_script),
...
def extract_frames(npz_path: Path, out_dir: Path) -> tuple[int, int]:
...
def process_clip(subset: str, seq_id: int, fname: str, h5_dir: Path) -> bool:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        extract_frames(npz_path, frames_dir)
...
def write_tapip3d_configs(subsets: list[str]) -> None:
    """Write TAPIP3D annotation + dataset configs for MegaSAM annotations."""
    for subset in subsets:
        h5_dir = TAPIP3D_ANNO_ROOT / f"{subset}_megasam_minival" / "megasam"
        anno_dir = TAPIP3D_ANNO_CFG_ROOT / f"{subset}_megasam_minival"
        ...
        # Derive dataset eval config from the da3 one
        ...
        if ref_cfg.exists():
            new_text = ref_cfg.read_text().replace(
                ...
                f"override_anno: {subset}_megasam_minival/megasam",
            )
            out_cfg = TAPIP3D_ROOT / "configs" / "dataset" / "eval" / f"{subset}_megasam_minival.yaml"
            out_cfg.write_text(new_text)

    dataset_defaults = "\n".join(
        f"  - dataset/eval/{s}_megasam_minival@test_datasets.{s}_megasam_minival"
        for s in subsets
    )
    top = TAPIP3D_ROOT / "configs" / "tapip3d_megasam_minival_eval.yaml"
    top.write_text(
...
def main():
    ...
    ap.add_argument("--subsets", nargs="+", default=["drivetrack", "pstudio"],
    ...
    for subset in args.subsets:
        ...
        for seq_id, fname in clips_to_run:
            ...
            ok = process_clip(subset, seq_id, fname, h5_dir)
            if ok:
                n_ok += 1
            else:
                n_fail += 1

        print(f"[{subset}] done: {n_ok} new, {n_skip} cached, {n_fail} failed → {h5_dir}")
        n_done = len(list(h5_dir.glob("*.confirm")))
        if n_done > 0:
            processed_subsets.append(subset)

    if processed_subsets:
        write_tapip3d_configs(processed_subsets)
    else:
